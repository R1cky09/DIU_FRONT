...
export default function Home() {
  ...
  const prev = () => setCurrentIndex(i => (i - 1 + antiguos.length) % antiguos.length);
  ...
  return (
    <div className="space-y-12">
      ...
      <section className="max-w-4xl mx-auto">
        <div className="relative h-80 rounded-xl shadow-lg overflow-hidden">
          {antiguos.map((e, idx) => (
            ...
            >
              <img src={e.imagen} alt={e.titulo} className="w-full h-full object-cover" />
          ...
          {/* Flechas */}
          <button onClick={prev} className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-30 hover:bg-opacity-50 text-white text-2xl p-2 rounded-full shadow-md focus:outline-none z-20">‹</button>
          <button onClick={next} className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-30 hover:bg-opacity-50 text-white text-2xl p-2 rounded-full shadow-md focus:outline-none z-20">›</button>
          {/* Indicadores */}
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2 z-20">
            {antiguos.map((_, i) => (
              <span key={i} className={`w-3 h-3 rounded-full ${i === currentIndex ? 'bg-white' : 'bg-white/50'}`}/>
            ))}
          </div>
        </div>
      </section>
      ...
      <div className="flex flex-wrap gap-4 justify-center">
        <input type="text" placeholder="Buscar por título" value={search} onChange={e => setSearch(e.target.value)} className="p-2 border rounded w-64" />
      ...
      {search || tipo || modalidad || campus ? (
        <section className="flex flex-wrap justify-center gap-6">
          {filtrados.length > 0 ? filtrados.map(evt => <EventCard key={evt.id} evento={evt} />) : <p className="text-center text-gray-500">No se encontraron eventos.</p>}
        </section>
      ) : (
        <>
          <section>
            ...
            <div className="flex flex-wrap justify-center gap-6">
              {recomendados.map(evt => <EventCard key={evt.id} evento={evt} />)}
            </div>
          </section>
          <section>
            <h2 className="text-2xl font-bold mb-4">Novedades</h2>
            <div className="flex flex-wrap justify-center gap-6">
              {novedades.map(evt => <EventCard key={evt.id} evento={evt} />)}
            </div>
          </section>
        </>
      )}
